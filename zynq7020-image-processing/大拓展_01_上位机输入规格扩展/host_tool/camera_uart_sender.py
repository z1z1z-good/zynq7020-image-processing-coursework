...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=PROC_SIZES,
        default="128x72",
        help="Host processing resolution; non-128x72 choices are mapped back into "
        "the fixed output size before transmission",
    )
    parser.add_argument(
        "--fill-color",
...
def prepare_frame(
    ...
    (e.g. (160, 90), (144, 108) or (64, 36)) first and then nearest-neighbour
    maps it back into the fixed width x height transmit frame. The HDMI image
    still uses the hardware-compatible wire format. None keeps the full output
    resolution.
...
def parse_proc_size(value: str, width: int, height: int) -> tuple[int, int] | None:
    """Map a --proc-size choice such as '160x90' or '64x36' to a content_size for
