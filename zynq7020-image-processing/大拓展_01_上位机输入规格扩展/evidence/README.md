...
- 目标处理尺寸 `--proc-size`：`128x72`、`160x90`、`144x108` 或 `64x36`。前三个对应 C 档参考尺寸；
  `64×36` 用于低处理尺寸对比。无论选哪种，最终 **发送给 FPGA 的帧恒为 `128×72`**。
...
| `ext_proc_size.png` | 同一 `640×480` 源在不同 `proc-size` 下的 `128×72`(×10) 对比，`64x36` 明显块状 |
| `ext_scaling_matrix.txt` | 3 fit 模式 × 3 原始尺寸 × 4 处理尺寸 = 36 组：输出形状/包长/帧头核对；letterbox 主体框宽高比与填充像素；center-crop 无黑边核对 |
| `ext_offline_tests.txt` | 两套离线测试 standalone 运行结果（`8/8` + `4/4` = `12/12` 全过）与三不变量摘要 |
...
| 2 | 可选目标处理尺寸（3 种以上） | 完成 | CLI/GUI `--proc-size {128x72,160x90,144x108,64x36}`；前三个覆盖 C 档参考尺寸 |
...
- 离线测试覆盖 C 档参考尺寸：
  - 形状：三 fit 模式 × 三原始尺寸 × 四处理尺寸输出均为 `(72, 128, 3)` `uint8`。
  ...
  - `proc-size` 覆盖 `128x72`、`160x90`、`144x108`、`64x36`；其中 `64×36` 输出 `2×2` 像素块均匀（更粗）且仍为合法 `128×72` 帧。
...
3. 目标处理尺寸对比：`--proc-size 128x72`、`160x90`、`144x108` 与 `64x36`（前三个覆盖 C 档参考尺寸，后者看 HDMI 画面变粗）。
...
- `proc 128x72`、`160x90`、`144x108`、`64x36` 的 HDMI 或上位机日志/预览对比照片。
