...
本文件由原 `coursework/README.md` 拆出，保留实验 0 到实验 5 的详细远程开发历史、分支记录和阶段结论。当前主线入口见 [`../README.md`](../README.md)，证据索引见 [`../evidence/README.md`](../evidence/README.md)。
...
完整路线、环境复核、风险和验收要求见
[单人 AI 辅助实施报告](ZYNQ7020图像处理课程设计_单人AI辅助实施报告.md)。

开发板暂不可用时，实验 1 至实验 5 的可执行范围、分批顺序、交付物和现场门禁见
[无板开发分批计划](BOARDLESS_DEVELOPMENT_PLAN.md)。

「上位机 → PS → BRAM → PL → HDMI」无板卡软硬件协同仿真的可复用流程（实验 3 建立，
实验 4、实验 5 应复用）见 [无板卡协同仿真方法论](BOARDLESS_COSIM_METHODOLOGY.md)。
...
- 可复用的启动 Prompt 和回传模板见
  [`REMOTE_HARDWARE_WORKFLOW_PROMPT.md`](REMOTE_HARDWARE_WORKFLOW_PROMPT.md)。
