#!/usr/bin/env python3
"""实验 3 无板卡软硬件协同仿真的 Python 环节。

子命令：
  gen            用真实 camera_uart_sender.build_frame_packet 生成 UART 字节流，
                 与本地编码器逐字节比对（证明上位机打包一致），写出 frame_stream.bin
                 和 golden_fb.hex（期望 framebuffer）。
  check-fb       比对 PS 主机模型产出的 framebuffer 与 golden（逐像素）。
  render-compare 读 RTL 仿真捕获的 1280x720 HDMI 像素，重建 PNG，并与由 golden
                 framebuffer 计算的期望画面逐像素比对。

复用 ../generate_exp03_expected.py 的纯标准库实现；真实上位机脚本依赖 numpy，
cv2/serial 在导入时用空桩绕过（本环节不打开真实串口/摄像头）。
"""
...
SOBEL3_DIR = TOOLS_DIR.parent
ZYNQ_DIR = SOBEL3_DIR.parent
HOST_DIR = ZYNQ_DIR / "host_camera_uart"


def _load_generate():
    spec = importlib.util.spec_from_file_location(
        "generate_exp03_expected", TOOLS_DIR / "generate_exp03_expected.py"
    )
...
def cmd_gen(args: argparse.Namespace) -> None:
    ...
    print(f"EXP03_COSIM_HOST_PACK=match bytes={len(real_stream)}")
    print(f"EXP03_COSIM_GOLDEN_FB_WORDS={len(golden_fb)}")
    print(f"EXP03_COSIM_ERROR_CASES={len(error_cases)}")
...
def cmd_check_fb(args: argparse.Namespace) -> None:
    ...
    print(f"EXP03_COSIM_FB=match words={len(golden)}")
...
def cmd_render_compare(args: argparse.Namespace) -> None:
    ...
    golden_rows = g.render_expected_hdmi(golden_fb)  # list[bytes]，每行 out_w*3
    ...
    print(f"EXP03_COSIM_PNG=match pixels={expected_px}")
