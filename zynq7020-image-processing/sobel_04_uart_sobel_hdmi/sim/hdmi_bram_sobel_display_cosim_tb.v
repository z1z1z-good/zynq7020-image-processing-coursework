...
// 实验 4 联合仿真 testbench（全分辨率渲染捕获）。
// 行为 BRAM 载入由真实 PS 主机模型产出的 fb_from_ps.hex（即真实上位机字节流经真实
// main.c receive_frame 写出的原始 RGB framebuffer），驱动 hdmi_bram_sobel_display 以
// 默认 1280x720 时序、10x 放大、默认阈值 80 和绿色边缘输出。PL 在每帧开始扫描 BRAM、
// 做 gray + Sobel，把边缘强度写入 edge_mem，再按 edge_pixel >= EDGE_THRESHOLD 映射为
// EDGE_COLOR / 黑色。testbench 把第一整帧有效像素按光栅顺序捕获到 hdmi_capture.hex，
// 交给 exp04_cosim.py render-compare 重建 PNG 并与软件 golden（gray+Sobel+彩色映射）比对。
...
// 默认参数（与综合一致：1280x720、10x、EDGE_THRESHOLD=80、EDGE_COLOR=24'h00ff00）。
    ...
    if (!rst && vs && !previous_vs) begin
        ...
        if (frame_count == 1) begin
            ...
            if (captured == H_ACTIVE * V_ACTIVE) begin
                $display("EXP04_COSIM_CAPTURE=ok pixels=%0d", captured);
                test_passed = 1'b1;
            end else begin
                $display("EXP04_COSIM_CAPTURE=bad pixels=%0d expected=%0d",
