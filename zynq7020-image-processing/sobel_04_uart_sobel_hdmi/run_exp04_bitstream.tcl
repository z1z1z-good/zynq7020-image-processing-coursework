...
set src_dir [file join $script_dir sobel_04_uart_sobel_hdmi.srcs sources_1]
...
set xdc_dir [file join $script_dir sobel_04_uart_sobel_hdmi.srcs constrs_1 new]
set evidence_dir [file normalize [file join $script_dir .. coursework evidence 05_uart_sobel]]
...
# 解析当前 Vivado 版本中某 IP 的最新可用 VLNV，避免硬编码 2017.4 版本在 2023.2 下不被支持。
proc latest_ip {name} {
...
create_project -force exp04_build $build_dir -part xc7z020clg400-2
set_property target_language Verilog [current_project]

# ---------- Block design：PS7 + SmartConnect + AXI BRAM Controller + Block Memory ----------
# 在隔离 build 工程内全新重建 BD，不触碰仓库中已提交的 2017.4 BD。与实验 3 同源。
...
# ---------- PL RTL 顶层、PL Sobel 与 HDMI 归档源码 ----------
# 实验 4 相对实验 3 增加 hdmi_bram_sobel_display + rgb_to_gray + sobel_core；
...
puts "EXP04_STAGE=synthesis"
synth_design -top top -part xc7z020clg400-2
report_utilization -file [file join $evidence_dir exp04_utilization.txt]

puts "EXP04_STAGE=implementation"
opt_design
place_design
phys_opt_design
route_design
...
report_timing_summary -delay_type min_max -report_unconstrained \
    ...
    -file [file join $evidence_dir exp04_timing_summary.txt]
report_drc -file [file join $evidence_dir exp04_drc.txt]

set bit_file [file join $build_dir top.bit]
puts "EXP04_STAGE=bitstream"
write_bitstream -force $bit_file
puts "EXP04_BITSTREAM=$bit_file"
...
puts "EXP04_XSA=$xsa_file"
puts "EXP04_BUILD=passed"
