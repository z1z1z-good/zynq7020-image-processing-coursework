/*
 * 实验 4 PS 接收路径的主机模型（联合仿真用）。
 *
 * 通过 #include 直接纳入未修改的 ps_uart_sobel_bram_app/src/main.c（仅把它的 main()
 ...
 * 然后调用真实的 receive_frame()，把得到的 framebuffer 以 0x00RRGGBB 32 位词导出为 hex。
 * 用于证明“真实上位机字节流 -> 真实 PS C 解析 -> 期望 framebuffer（原始 RGB）”。
 *
 * 实验 4 的 PS 接收逻辑与实验 3 完全一致（Sobel 在 PL 端做，不在 PS），因此本模型与
 * 实验 3 的 ps_protocol_model.c 共用同一套主机桩，只是改为纳入实验 4 的 main.c。
 *
 * 这是无板卡软硬件协同仿真的 PS 环节，不是在真实 Zynq 上运行。
 * 用主机 gcc 编译：
 *   gcc -Dmain=ps_app_main_unused \
 *       -I <sobel_04>/tools/ps_syntax_check/include \
 *       -I <sobel_04>/ps_uart_sobel_bram_app/src \
