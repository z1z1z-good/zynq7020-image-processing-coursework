...
| 7 | 创新点 | 必须 | 不改硬件，独立大拓展目录；三种 fit；C 档三种参考尺寸；保持协议不变 | [zynq7020-image-processing/coursework/evidence/照片证明/大拓展最终验收视频截图/strech_128x72.png](../coursework/evidence/照片证明/大拓展最终验收视频截图/strech_128x72.png) 等最终验收截图 |
| 8 | 实现原理 | 必须 | `prepare_frame -> build_frame_packet -> UART -> sobel_05` | 简图 + 帧头 `55 aa 80 00 48 00 18` |
| 9 | 仿真/软件验证 | 必须 | 12/12 测试通过；36 组矩阵；包长和帧头不变 | [zynq7020-image-processing/大拓展_01_上位机输入规格扩展/evidence/ext_offline_tests.txt](../大拓展_01_上位机输入规格扩展/evidence/ext_offline_tests.txt) 摘要、[zynq7020-image-processing/大拓展_01_上位机输入规格扩展/evidence/ext_scaling_matrix.txt](../大拓展_01_上位机输入规格扩展/evidence/ext_scaling_matrix.txt) 摘要 |
| 10 | 上板展示结果 | 必须 | 三种 fit 策略、C 档三种参考尺寸和低处理尺寸最终效果 | [zynq7020-image-processing/coursework/evidence/照片证明/大拓展最终验收视频截图/](../coursework/evidence/照片证明/大拓展最终验收视频截图/) 选 5 张 |
...
创新点 4：处理尺寸可选  
支持 `128x72`、`160x90`、`144x108` 和 `64x36` 四种处理尺寸。前三种对应 C 档参考尺寸；`64x36` 会产生更粗的块状效果。最终发送帧仍保持 `128x72`，因此不影响 FPGA 接收协议。
...
避免说“创新点很多”这类空话。按“硬件不变、PC 适配、三种策略、三种评分参考尺寸”四点讲即可。
...
综合扩展先完成离线验证，再进行上板演示。离线验证覆盖 3 种原始尺寸、3 种 fit 策略和 4 种处理尺寸，共 `3 x 3 x 4 = 36` 组组合。处理尺寸包含 `128x72`、`160x90`、`144x108` 三种 C 档参考尺寸，以及 `64x36` 低处理尺寸对比。每组输出最终都满足固定形状 `(72,128,3)`，打包后帧长均为 `27943B`，帧头均为 `55 aa 80 00 48 00 18`。
...
4. `128x72`、`160x90`、`144x108` 覆盖 C 档参考尺寸；`64x36` 处理尺寸输出更粗。
...
左侧放测试表，右侧放竖屏 fit 策略对比图。不要直接贴整段日志，提炼成 12/12 和 36 组矩阵即可。
...
最终验收重点展示上位机输入规格扩展的实际效果。扩展不修改 FPGA 侧 PS/BRAM/PL/HDMI 链路，而是在 PC 端把不同尺寸输入统一适配为固定 `128x72 RGB888` 帧。最终截图能够直接展示三种 fit 策略、三种 C 档参考尺寸和低处理尺寸对比。
