...
资料来源：老师原仓库 `zhouxzh/FPGA-course/zynq7020-image-processing/README.md`，以及本仓库 `reports/最终报告.md`、`coursework/evidence/` 下现有证据。老师原仓库地址：<https://github.com/zhouxzh/FPGA-course/tree/main/zynq7020-image-processing>
...
| 序号 | 要求 | 已准备材料 |
| --- | --- | --- |
| 1 | 第一周实验 0 到实验 4 基础扩展选择和结果，实验 5 控制功能结果 | `reports/最终报告.md` 第 4 节；现场照片在 `coursework/evidence/02..06_*` |
...
| 5 | 第二周综合扩展仿真截图、波形截图或软件验证结果 | `coursework/evidence/07_input_scaling_extension/*.png`、`ext_offline_tests.txt`、`ext_scaling_matrix.txt` |
| 6 | Vivado 资源利用率、时序结果和 bitstream 生成结果 | 最终报告第 7 节；第二周扩展复用实验 5 bitstream，资源/时序同实验 5 |
| 7 | 上板演示现象、照片或视频截图 | `coursework/evidence/09_final_demo/*.jpg` |
...
| 3 | 系统总体架构 | 必须 | PC 上位机、UART、PS、AXI BRAM、PL、HDMI | `reports/最终报告.md` 第 2 节 Mermaid 图可重画 |
...
| 7 | 创新点 | 必须 | 不改硬件，PC 端统一适配；三种 fit；两种处理尺寸；保持协议不变 | `ext_fit_modes_landscape.png` |
| 8 | 实现原理 | 必须 | `prepare_frame -> build_frame_packet -> UART -> sobel_05` | 简图 + 帧头 `55 aa 80 00 48 00 18` |
| 9 | 仿真/软件验证 | 必须 | 11/11 测试通过；18 组矩阵；包长和帧头不变 | `ext_offline_tests.txt` 摘要、`ext_scaling_matrix.txt` 摘要 |
| 10 | 上板展示结果 | 必须 | 原图/灰度/边缘/叠加，阈值 120 对比 | `09_final_demo` 选 4 张 |
...
| 用途 | 文件 |
| --- | --- |
| RTL 仿真关键波形 | `coursework/evidence/01_rtl_sim/exp00_key_waveform.png` |
| 实验 1 HDMI 固定图 | `coursework/evidence/02_hdmi_pattern/exp01_hdmi_field_20260618.jpg` |
| 实验 4 UART Sobel | `coursework/evidence/05_uart_sobel/exp04_uart_sobel_field_20260618.jpg` |
| 实验 5 叠加模式 | `coursework/evidence/06_pc_control/exp05_mode_overlay_field_20260618.jpg` |
| 大拓展横屏 fit 策略 | `coursework/evidence/07_input_scaling_extension/ext_fit_modes_landscape.png` |
| 大拓展竖屏 fit 策略 | `coursework/evidence/07_input_scaling_extension/ext_fit_modes_portrait.png` |
| 大拓展处理尺寸对比 | `coursework/evidence/07_input_scaling_extension/ext_proc_size.png` |
...
| 用途 | 文件 |
| --- | --- |
| 实验 2 固定图 Sobel | `coursework/evidence/03_hdmi_sobel/exp02_hdmi_sobel_field_20260618.jpg` |
| 实验 3 UART 原图带边框 | `coursework/evidence/04_uart_hdmi/exp03_uart_hdmi_field_20260618.jpg` |
| 实验 5 原图/灰度/边缘 | `coursework/evidence/06_pc_control/exp05_mode_original_field_20260618.jpg`、`exp05_mode_gray_field_20260618.jpg`、`exp05_mode_edge_threshold_sparse_field_20260618.jpg` |
| 阈值 120 最终演示 | `coursework/evidence/09_final_demo/final_live_gray_threshold120_gui_20260618.jpg`、`final_live_edge_threshold120_gui_20260618.jpg`、`final_live_overlay_threshold120_gui_20260618.jpg` |
...
3. 如果老师要求演示视频，需从 `09_final_demo` 现场过程或重新录屏中截取。
4. 若需要正式提交最终报告，确认 `reports/最终报告.md` 和 `coursework/evidence/07_input_scaling_extension/*` 已纳入 Git。
...
`coursework/evidence/09_final_demo/final_live_overlay_gui_board_20260618.jpg`
...
- `coursework/evidence/01_rtl_sim/exp00_key_waveform.png`
- `coursework/evidence/02_hdmi_pattern/exp01_hdmi_field_20260618.jpg`
- `coursework/evidence/05_uart_sobel/exp04_uart_sobel_field_20260618.jpg`
- `coursework/evidence/06_pc_control/exp05_mode_overlay_field_20260618.jpg`
...
- `coursework/evidence/02_hdmi_pattern/exp01_hdmi_field_20260618.jpg`
- `coursework/evidence/03_hdmi_sobel/exp02_hdmi_sobel_field_20260618.jpg`
- `coursework/evidence/06_pc_control/exp05_mode_overlay_field_20260618.jpg`
...
`coursework/evidence/07_input_scaling_extension/ext_fit_modes_landscape.png`
...
- `coursework/evidence/07_input_scaling_extension/ext_fit_modes_landscape.png`
- `coursework/evidence/07_input_scaling_extension/ext_proc_size.png`
...
建议重画流程图；也可放 `ext_cli_help.txt` 中参数截图，但优先用流程图。
...
- `coursework/evidence/07_input_scaling_extension/ext_fit_modes_portrait.png`
- `coursework/evidence/07_input_scaling_extension/ext_offline_tests.txt` 的测试结果可截图或手工做成表格
...
综合扩展复用实验 5 的硬件链路，现场重点验证显示模式和控制链路稳定性。最终演示能够显示原图、灰度图、边缘图和红色边缘叠加图，并能够在不同阈值下观察边缘结果变化。

展示结果：

1. 原图模式：显示 PC 输入图像本身。
2. 灰度模式：显示 RGB 转灰度后的结果。
3. 边缘模式：显示 Sobel 二值边缘图。
4. 叠加模式：在原图上叠加红色边缘。
5. 阈值 120：边缘更稀疏，说明阈值控制有效。

**推荐图片**

必放 4 张：

- `coursework/evidence/09_final_demo/final_live_original_gui_board_20260618.jpg`
- `coursework/evidence/09_final_demo/final_live_gray_gui_board_20260618.jpg`
- `coursework/evidence/09_final_demo/final_live_edge_gui_board_20260618.jpg`
- `coursework/evidence/09_final_demo/final_live_overlay_gui_board_20260618.jpg`

可选补充：

- `coursework/evidence/09_final_demo/final_live_edge_threshold120_gui_20260618.jpg`
- `coursework/evidence/09_final_demo/final_live_overlay_threshold120_gui_20260618.jpg`

**版式建议**

做成 2x2 图片矩阵，每张图片下方写模式名：原图、灰度、边缘、叠加。阈值 120 图片如果放不下，可放到备注或下一页角落。

**讲解提示**

这页是答辩视觉重点。不要讲太长，直接说明四种模式都能上板显示，阈值变化也能反映在边缘结果中。
